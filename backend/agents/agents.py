...
from agents.state import ProtocolState, add_agent_note, add_draft_version
...
{iteration_context}""")
        ...
        # Add note to blackboard
        state = add_agent_note(
            state,
            self.name,
            f"Created draft version {state['current_version'] + 1}",
            priority="info"
        )
    ...
    async def decide(self, state: ProtocolState) -> Dict[str, Any]:
        ...
        if iteration >= max_iterations:
            state = add_agent_note(
                state,
                self.name,
                f"Reached max iterations ({max_iterations}). Finalizing current draft.",
                priority="warning"
            )
            return {
                "next_agent": "HALT",
        ...
        if quality_ok and state.get("current_draft"):
            ...
            state = add_agent_note(
                state,
                self.name,
                "Draft meets quality thresholds. Ready for human review.",
                priority="info"
            )
            # Log activity
            log_agent_activity(
        ...
        if needs_revision and iteration < max_iterations:
            state = add_agent_note(
                state,
                self.name,
                f"Quality scores below threshold. Requesting revision (iteration {iteration + 1}/{max_iterations})",
                priority="warning"
            )
            return {
                "next_agent": "Drafter",
